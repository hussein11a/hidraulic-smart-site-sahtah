...
const InteractiveServices: React.FC<InteractiveServicesProps> = ({ services, isDarkMode }) => {
  ...
  const toggleServiceDetails = (service: Service) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-16">
          {services.map((service, index) => (
            <div key={service.id} className="relative">
              ...
              >
                ...
                <div className="relative z-10 p-10">
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Expand Button */}
                    <button 
                      onClick={() => toggleServiceDetails(service)}
                      className={`w-full flex items-center justify-center gap-3 px-6 py-4 rounded-2xl transition-all duration-300 font-bold text-lg border-2 ${
                        selectedService?.id === service.id
                          ? (isDarkMode 
              ...
              {/* Expanded Details */}
              {selectedService?.id === service.id && (
                <div className={`absolute top-full left-0 right-0 z-30 mt-4 p-8 rounded-3xl border-2 shadow-2xl transition-all duration-500 transform ${
                  isDarkMode 
